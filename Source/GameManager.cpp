...
namespace Game {
    ...
    void GameManager::run(sf::RenderWindow& i_window) {
    ...
    bool GameManager::collisionOfBirdWithPipes(const Bird& bird, const std::vector<Pipe>& pipes)  {
        ...
        for (const Pipe& curr_pipe : pipes) {
            // Check if the pipe is behind the bird, if not, move to the next pipe
            if (curr_pipe.getXPosition() < birdRearRange - 1) 
                continue; 
            // Check if the pipe is range to collide with the bird, if not, return to gameplay
